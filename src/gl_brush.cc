...
void GL_DrawSurfaces(brush_model_t *brushmodel, texchain_t chain)
{
	...
	glDisableVertexAttribArray(brush_LMTexCoordsAttrib);
...
void GL_DrawBrushModel(entity_t *ent)
{
	...
	R_ClearTextureChains(brushmodel, chain_model);
	R_ClearLightmapPolys(brushmodel);

	// calculate dynamic lighting for bmodel if it's not an instanced model
	if (brushmodel->firstmodelsurface == 0)
		R_PushDlights(&brushmodel->nodes[brushmodel->hulls[0].firstclipnode], brushmodel->surfaces);
