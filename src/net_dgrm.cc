...
static const char *Strip_Port(const char *host)
{
	...
	if (!host || !*host)
		return host;
	strlcpy(noport, host, sizeof(noport));
...
static void Test_f(void)
{
	...
	JustDoIt: testSocket = dfunc.OpenSocket(0);
...
static void Test2_Poll(void *unused)
{
	...
	*((int *) net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
	...
	Reschedule: SchedulePollProcedure(&test2PollProcedure, 0.05);
	return;

	Error: Con_Printf("Unexpected repsonse to Rule Info request\n");
	Done:
...
static void Test2_f(void)
{
	...
	JustDoIt: test2Socket = dfunc.OpenSocket(0);
...
static qsocket_t *_Datagram_Connect(const char *host)
{
	...
	ErrorReturn: NET_FreeQSocket(sock);
	ErrorReturn2:
	dfunc.CloseSocket(newsock);
	if (m_return_onerror)
	{
